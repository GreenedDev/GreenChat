...
public class ToggleChatCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (GreenFilter.isChatEnabled == null) {
            Chat.broadcast(Utils.replacePlayer(plugin.configKeys.getLang("commands.toggle-chat.someone-disabled-chat"), sender.getName()));
            GreenFilter.isChatEnabled = sender.getName();
        } else {
            Chat.broadcast(Utils.replacePlayer(plugin.configKeys.getLang("commands.toggle-chat.someone-enabled-chat"), sender.getName()));
